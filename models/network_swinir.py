...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        ### for liif ###
        imnet_in_dim = embed_dim
        imnet_in_dim += 2 # attach coord
        self.imnet = MLP(imnet_in_dim, 3, hidden_list=[256]*4)
        ### for liif ###
    ...
    def forward(self, x, target_size):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_last(x)
        elif self.upsampler == 'liif':
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x)) + x # n,c,pad_h,pad_w
            coord = make_coord((target_size[-2], target_size[-1])).to(x.device).unsqueeze(0).expand(x.size(0),-1,-1) # n,pad_sh*pad_sw,2
            x = self.query_rgb(x, coord).permute(0,2,1).view(x.size(0),-1,target_size[-2], target_size[-1]) # n,c=3,pad_sh,pad_sw
        else:
            # for image denoising and JPEG compression artifact reduction
